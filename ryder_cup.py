...
with tab_rounds:
    ...
    # Debug line to confirm matches exist
    st.caption(f"Debug: Matches detected = {len(st.session_state.matches)}")

    if len(st.session_state.matches) == 0:
        st.warning("No matches created yet. Use the Setup Matches tab to create them.")
    else:
        round_options = [
            "Round 1 – Foursomes",
            "Round 2 – Fourball",
        ...
        round_tab = st.selectbox("Choose Round", round_options)

        # FIXED: Proper round key mapping
        round_map = {
            "Round 1 – Foursomes": "R1",
            "Round 2 – Fourball": "R2",
            "Round 3 – Greensomes": "R3",
            "Round 4 – Singles (1v1)": "R4"
        }
        r_key = round_map.get(round_tab, "R1")  # fallback

        cols = st.columns(2 if "Singles" not in round_tab else 1)
        for i, col in enumerate(cols if "Singles" not in round_tab else [st.container()] * 4):
            with col:
                m_key = f"{r_key}-M{i+1}"
                ...
                if fmt == "Singles":
                    ...
                    if st.button("Save & Calculate", key=f"save_p_{m_key}"):
                        ...
                        match["completed"] = True
                        for p in match["players_w"]:
                            st.session_state.player_points[p] += pts_w
                        for p in match["players_k"]:
                            st.session_state.player_points[p] += pts_k
